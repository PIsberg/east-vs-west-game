...
**Everything here must stay on the instanced particle path** — fast weapons fire many times a second, and a special particle (`isShockwave`, beams, corpses…) costs a React component and a draw call each. Measured load: idle 0, ~317 particles at peak in a 48-unit point-blank firefight, against the `MAX_PARTICLE_INSTANCES` budget of 2048. Keep new per-shot effects inside that budget, and remember the counts in `FIRE_FX` are *per shot*.
