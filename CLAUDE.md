...
**Winter's frozen river** (`TerrainObject.frozen` on the river segments): foot units cross the ice anywhere at `ICE_CROSS_MULT` speed — the movement block short-circuits the bridge detour — and skip the wading range penalty (walked, not waded; the cost is being in the open). Vehicles still need bridges. Gunboats treat ice as dry land (`spawnUnit` veto + the CPU picket filters frozen segments). Weather is climate-aware: the single `rollNextWeather(map)` helper (module scope, used by BOTH roll sites — initial forecast and in-game re-roll) makes Winter snow instead of rain and keeps snow off the Desert entirely (fog there reads as dust haze). `__ewDebug.riverSegs`/`bridges` expose the geometry for movement probes.
