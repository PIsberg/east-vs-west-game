...
- **Taking damage** — only *occupied* houses are targets; an empty one is indestructible cover and rounds pass it by. Direct fire resolves against the footprint in the projectile loop (guarded `occupant != null && occupant !== p.team`, so a garrison never shoots its own house); splash goes through `damageBuilding` inside `damageBridges`. Attackers with no unit target fall back to shooting the nearest enemy-held house (a synthetic ground target in the standard-targeting block) — which is how a garrison that's tucked away still gets stormed.
- **Damage states** — `health` fraction drives `state`: `normal` → `broken` → `burning` (coughs smoke) → collapse. At 0 HP the house **collapses**: `BUILDING_COLLAPSE_SURVIVE` of the garrison scrambles clear (hurt, re-added to `unitsRef`), the rest are counted lost, and it becomes permanent `burnt` rubble (neutral, no longer occupiable).
