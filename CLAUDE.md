...
GLB unit models (`public/models/*.glb`, Quaternius CC0): `SoldierModel`/`TankModel` in `GameScene.tsx` load via drei `useGLTF`, clone per unit with `SkeletonUtils.clone` (required for skinned meshes), tint team-marked materials (`Swat`/`Main`), and drive the models' own animation clips via `useAnimations` (soldier: Run/Idle_Gun/Idle_Gun_Shoot; tank: Tank_Forward kept playing-but-paused when stationary — a stopped action reverts skinned tracks to a coiled bind pose). Scales are empirical constants (`SOLDIER_SCALE`/`TANK_SCALE`) — bind-pose bounding boxes are useless because the source armatures bake large transforms. Remaining unit types still use the primitive builds below.
