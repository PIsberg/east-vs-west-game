...
(async () => {
  ...
  // seed=101: the forecast roll is seeded, and unpinned it is a coin flip —
  // the chip only appears when the pre-roll comes up non-clear (~65%), so this
  // test failed on a working build roughly one run in three. 101 rolls "rain
  // in 10s" every time (same seed smoke10 pins for its river).
